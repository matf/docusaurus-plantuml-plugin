...
/**
 * The frame's layout: three stacked rows — controls, picture, minimap toggle.
 *
 ...
 * the geometry — the row structure in the markup, and the stylesheet's promise to keep those
 * rows in flow.
 */
...
describe('the frame is three stacked rows', () => {
  it('puts the controls above the picture and the minimap toggle below it', async () => {
    await renderReady();

    const rows = Array.from(stage().children);
    expect(rows).toHaveLength(3);

    const [controls, body, minimapBar] = rows as HTMLElement[];
    expect(controls).toContainElement(screen.getByRole('group', {name: /zoom controls/}));
    expect(body).toContainElement(viewport());
    expect(minimapBar).toContainElement(screen.getByRole('button', {name: 'Show minimap'}));
    ...
    expect(minimapBar?.compareDocumentPosition(viewport())).toBe(Node.DOCUMENT_POSITION_PRECEDING);
  });
  ...
  it('anchors the minimap itself to the picture, since the reader asked for it', async () => {
    ...
    // Inside the diagram row, not a fourth row: the map is the one control that may cover the
    // picture, and it is positioned within the box it mirrors.
    expect(minimapPanel()?.parentElement).toBe(stage().children[1]);
    expect(stage().children).toHaveLength(3);
  });

  it('drops the minimap row with the picture when the source view is shown', async () => {
    const user = userEvent.setup();
    await renderReady();
...
describe('the stylesheet keeps the rows out of the picture', () => {
  ...
  it('leaves both control rows in flow, so neither can be painted over the diagram', () => {
    ...
    expect(declarations('.minimapBar')).not.toMatch(/position:/);
  });
